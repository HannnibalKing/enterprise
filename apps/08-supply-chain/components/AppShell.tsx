...
interface User{id:string;name:string;email:string;avatar:string;role:UserRole;title:string;}
const NAV=[
...
function Clock(){
  ...
  return <span style={{fontFamily:'monospace',fontSize:13,color:'var(--accent)',letterSpacing:'0.05em'}}>{t} UTC</span>;
}
    ...
    <div style={{display:'flex',height:'100vh',overflow:'hidden'}}>
      <aside style={{width:200,flexShrink:0,background:'var(--surface)',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column'}}>
        <div style={{padding:'18px 16px 14px',borderBottom:'1px solid var(--border)'}}>
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <div style={{width:32,height:32,borderRadius:7,background:'linear-gradient(135deg,#fb923c,#ea580c)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>⛓</div>
            <div><div style={{fontSize:11,fontWeight:800,letterSpacing:'0.07em',color:'var(--text)'}}>ATLAS</div><div style={{fontSize:9,letterSpacing:'0.12em',color:'var(--accent)'}}>CHAIN</div></div>
          </div>
        </div>
        <nav style={{flex:1,padding:'10px 8px',overflowY:'auto'}}>
          {NAV.map(item=>{const active=pathname===item.href||pathname.startsWith(item.href+'/');return(
            <a key={item.href} href={item.href} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',borderRadius:6,marginBottom:2,fontSize:13,fontWeight:active?700:500,color:active?'var(--accent)':'var(--text-soft)',background:active?'rgba(251,146,60,0.10)':'transparent',borderLeft:`2px solid ${active?'var(--accent)':'transparent'}`,transition:'all 0.15s',textDecoration:'none'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <header style={{height:48,flexShrink:0,background:'var(--surface)',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',padding:'0 22px',justifyContent:'space-between'}}>
          <span style={{fontSize:12,color:'var(--text-muted)',letterSpacing:'0.06em'}}>ATLAS CHAIN MANAGEMENT · <span style={{color:'var(--text-soft)'}}>Global Operations Center</span></span>
          <div style={{display:'flex',alignItems:'center',gap:14}}>
            ...
            <span style={{display:'inline-flex',alignItems:'center',gap:5,padding:'3px 9px',background:'var(--green-dim)',borderRadius:20,border:'1px solid rgba(52,211,153,0.25)',fontSize:11,fontWeight:700,color:'var(--green)'}}>
              <span style={{width:6,height:6,borderRadius:'50%',background:'var(--green)',animation:'pulse 2s infinite',display:'inline-block'}}/>LIVE TRACKING
            </span>
          </div>
        </header>
