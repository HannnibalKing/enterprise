...
const NAV = [
  ...
  { href: '/production', icon: '⚙️', label: 'Production'  },
  ...
  return (
    <div className="shell">
      <aside className="sidebar">
        <div className="sidebar-logo">FORGEMAX<span>Manufacturing Execution System</span></div>
        <nav className="sidebar-nav">
          {NAV.map(n => <Link key={n.href} href={n.href} className={`nav-item${pathname===n.href?' active':''}`}><span className="nav-icon">{n.icon}</span>{n.label}</Link>)}
        </nav>
      ...
      <div className="main">
        <header className="topbar">
          <span className="topbar-title">{NAV.find(n=>n.href===pathname)?.label ?? 'FORGEMAX'}</span>
          <div className="topbar-right"><span style={{ color: 'var(--accent)', fontWeight: 700 }}>{userRole.replace(/_/g,' ').toUpperCase()}</span></div>
        </header>
