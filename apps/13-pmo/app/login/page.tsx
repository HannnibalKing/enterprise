...
const USERS = ['xavier','diana','sam','tara','otto'];
export default function LoginPage() {
  const router = useRouter();
  const [username, setUsername] = useState('xavier');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    else setError('Invalid credentials. Try pmo123.');
  }
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      <div style={{ width: 360, padding: 36, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ fontSize: 28, fontWeight: 900, letterSpacing: '0.08em', color: 'var(--accent)', marginBottom: 4 }}>VANTAGE</div>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.2em', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Project Portfolio Management</div>
        </div>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)}
              placeholder="pmo123" style={{ width: '100%', padding: '10px 12px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 7, color: 'var(--text)', fontSize: 13 }} />
          </div>
