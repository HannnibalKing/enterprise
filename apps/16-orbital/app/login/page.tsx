    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)'}}>
      <div style={{width:380,padding:'2.5rem',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12}}>
        <div style={{textAlign:'center',marginBottom:'2rem'}}>
          <div style={{fontSize:'2rem',fontWeight:900,letterSpacing:4,color:'var(--accent)'}}>ORION TRACK</div>
          <div style={{color:'var(--text-soft)',fontSize:'.85rem',marginTop:4}}>Orbital Mechanics System</div>
        </div>
        <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
          ...
          <button type="submit" disabled={loading}
            style={{padding:'.75rem',background:'var(--accent)',color:'#fff',fontWeight:700,borderRadius:6,border:'none',cursor:'pointer',fontSize:'.9rem',letterSpacing:1,marginTop:'.5rem'}}>
            {loading ? 'AUTHENTICATING...' : 'ACCESS SYSTEM'}
          </button>
        </form>
        <div style={{marginTop:'1.5rem',fontSize:'.75rem',color:'var(--text-muted)',textAlign:'center'}}>
          kepler / orbital123 &bull; cassini / orbital123 &bull; newton / orbital123
        </div>
      </div>
    </div>
  );
